...
public class PessoaController {
	...
	public String salvarPessoa(@ModelAttribute(value="sala1") String salaId,
			@ModelAttribute(value="sala2") String sala2Id,
			@ModelAttribute(value="pessoa") Pessoa pessoa,
		...
		public String atualizarPessoa(@ModelAttribute(value="sala1") String salaId,
			...
			if(pessoaService.getPessoasNaSala1(sala).size() >= sala.getLotacao()) {
				attributes.addFlashAttribute("mensagem", 
						"A sala " + sala.getNome() + " está lotada");
	...
	//Deleta pessoa por id
	 ...
	 public String distribuirPessoas(
			 	@ModelAttribute(value="sala1") String salaId,
				@ModelAttribute(value="sala2") String sala2Id,
				@ModelAttribute(value="espacoCafe1") String espacoCafeId,
				@ModelAttribute(value="espacoCafe2") String espacoCafe2Id,
				RedirectAttributes attributes) {
		 
		 if(salaId.equals("") || 
				 sala2Id.equals("") || 
				 espacoCafeId.equals("") ||
				 espacoCafe2Id.equals("")) {
			 attributes.addFlashAttribute("mensagem", 
		...
		// mensagem de erro caso as salas sejam iguais
				 if(espacoCafeId.equals(espacoCafe2Id)) {
						attributes.addFlashAttribute("mensagem", 
								"Os espaços de café para intervalo não podem ser iguais");
						return "redirect:/distribuirForm";
					}
		 ...
		 if(total == 0) {
			 attributes.addFlashAttribute("mensagem", 
						"Não há pessoas sem salas ou sem espaços para intervalos");
				return "redirect:/distribuirForm";
		 }
	 ...
	 public String resetarPessoasAlocadas(RedirectAttributes attributes) {
		...
		//pega lista de pessoas sem salas 
		 ...
		 if(listaPessoas.size() != listaPessoasSemSalas.size()) {
			 ...
			 attributes.addFlashAttribute("mensagem", 
						"Todas pessoas foram desalocadas.");
				return "redirect:/listaPessoas";
		 }else {
			 attributes.addFlashAttribute("mensagem", 
						"Nenhuma pessoa está alocada.");
				return "redirect:/listaPessoas";
		 }	
	 }
}
